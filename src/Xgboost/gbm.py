    ...
    def fit(self,
        ...
        for i in range(self.num_boost_round):
            ...
            if self.eval_metric is None:
                logging.info("XGBoost round {iteration}".format(iteration=i))
            else:
                try:
                ...
                if not do_validation:
                    logging.info("XGBoost round {iteration}, train-{eval_metric}: {train_metric:.4f}".format(
                    ...
                    logging.info("XGBoost round {iteration}, train-{eval_metric}: {train_metric:.4f}, val-{eval_metric}: {val_metric:.4f}".format(
