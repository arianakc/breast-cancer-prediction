...
class ClassList(object):
    def __init__(self, Y):
        ...
        self.pred = np.ones(Y.shape, dtype="float32")
        self.grad = np.empty(Y.shape, dtype="float32")
        self.hess = np.empty(Y.shape, dtype="float32")

    def initialize_pred(self, init_pred):
        self.pred *= init_pred
